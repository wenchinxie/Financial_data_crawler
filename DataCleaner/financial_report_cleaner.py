...
class TWFinancialReport:
    ...
    def run_process(file):
        ...
        quarter_mapping = {"Q1": "03-31", "Q2": "05-15", "Q3": "08-15", "Q4": "11-15"}
    ...
    def BS_PL_CF_Parse(table, REQUIRE_COL=1) -> dict:
        ...
        periods = [
            time.find(class_="en").text
            for time in infos[1].find_all("th")[2 : 2 + REQUIRE_COL]
        ]
        ...
        for i, n in enumerate(infos[2:-2]):
            ...
            if code == "" or code.count("\u3000") < next_code.count("\u3000"):
                ...
                amts = [
                    amt.get_text().strip() for amt in n.select(".amt")[: len(d_list)]
                ]
