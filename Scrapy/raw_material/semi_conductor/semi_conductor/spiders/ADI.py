...
class SciSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        result = response.xpath(
            '//div[@id="Panel1"]/div[@class="div_content"]/div/table/tr/td/text()'
        ).getall()
        price_result = response.xpath(
            '//div[@id="Panel1"]/div[@class="div_content"]/div/table/tr/td/a/text()'
        ).getall()
        ...
        conn = sqlite3.connect(
            r"C:\Users\s3309\Website\Allium Tuberosum\at_django\raw_material\raw_material\RM.db"
        )
        c = conn.cursor()
        ...
        for i, j in zip(date_all, price_all):
            ...
            c.execute(
                "INSERT INTO Raw_material (Date,Material_name,Price) VALUES (?,?,?)",
                (date, table_name, price),
            )
            conn.commit()
